...
int main (void){
	...
	calculateRR( spkf_pointer, npkf_pointer, threshold1_pointer,
			threshold2_pointer, RR_low_pointer, RR_high_pointer,
			RR_miss_pointer, counter1_pointer, counter2_pointer,
			RRArray1_pointer, RRArray2_pointer,
			RRAverage1_pointer, RRAverage2_pointer,
			peaks_pointer, lastPeak_pointer, lastRPeak_pointer,
			RRMissCounter_pointer, totalRR_pointer,
			irregularHeartbeat_pointer, totalPeakCounter_pointer);
	 ...
	 * scanf("%d", &input);
	 * Due to time constraints, we did not manage to go futher
