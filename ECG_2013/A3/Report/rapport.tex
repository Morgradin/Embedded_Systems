...
\subsection{Functionality of the co-processor}
	...
	\\
	The above is made possible by sending all data in a single 32 bit signal over the bus - this report assumes that no value exceeds 16 bit size, which is equivalent to that no value exceeds $2^15 = 32768$. If this proves not to hold, the alternative could be to simply store all 30 values in the co-processor - this would increase the size of the co-processor, but it would make sure that only one data signal had to be sent over the bus.\\
