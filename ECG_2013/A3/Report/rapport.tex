...
\label{Prob1}
	...
	\subsubsection{Speed}
	The impact of speed is that if the co-processor is not fast enough, data will get bogged down and the user will be shown incorrect or old data - therefore, speed is essential. A way to increase the speed would be to increase the amount of transistors on the device, but that will increase both size and power.\\
	...
	Unfortunately, when using Gezel, the only way to actually calculate the speed is through using these very clock cycles, and therefore, this is what shall be used later on.\\
	\subsubsection{Power}
	...
	Size can be measured by simply counting the amount of components and bit widths and adding all these together.
	\\
