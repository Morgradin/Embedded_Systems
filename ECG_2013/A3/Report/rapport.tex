...
\ \\
\section*{Abstract}
The task of this assignment was to implement, integrate and analyse a co-processor for the MWI calculations from A2. This co-processor was designed and optimised for this specific task based on a definition of ``good performance''. It was to be implemented in such a way that it could communicate with a processor through a bus, the processor being the master and the co-processor being the slave. Once it was implemented and integrated, its performance was analysed for comparison with the previous system from A2, thereby finding out that its performance had actually decreased by moving the calculations to a dedicated co-processor.\\
...
\section{Introduction}
	After successfully implementing and integrating a dedicated processor to take care of the MWI filter, Medembed wanted a co-processor implemented and integrated into the same system. The task of this co-processor would be to further optimize the algorithm, letting the processor created in A2 take care of the more general tasks while the co-processor could be dedicated to the MWI algorithm, thereby allowing further optimization in terms of design.\\
	This co-processor were to be implemented in Gezel like the initial processor, and this report will discuss precisely how this co-processor were developed and integrated. It will also discuss the most important aspects of performance as well as the choices made on whether to optimise for speed, power or size.
...
\label{Prob1}
	The first thing that should be done would be to analyse the co-processor in terms of what good performance is. In terms of performance, there are three areas to consider: High speed, low area and low power. The task was to decipher which was more important for the current project, as each has a distinct impact on the co-processor.\\
